 ...
 */
@Service
public class SpringPayrollReportService {

    private static final Logger LOGGER = Logger.getLogger(SpringPayrollReportService.class.getName());

    /**
     * Generate PDF payroll summary report
     */
    public byte[] generatePayrollSummaryPdf(SpringPayrollService.PayrollSummary summary) {
        if (summary == null) {
    ...
    private PDType0Font loadSystemTTFFont(PDDocument document) {
        ...
        try (java.io.InputStream is = SpringPayrollReportService.class.getResourceAsStream("/fonts/DejaVuSans.ttf")) {
            if (is != null) {
