...
vi.mock('../hooks/useApi', () => {
  return {
    useApi: () => ({
      dataManagement: {
        generateInvoicePdf: vi.fn(async (_companyId: number, _invoiceId: number) => ({ success: true, message: 'PDF generated successfully' })),
        downloadInvoice: vi.fn(async (_companyId: number, _invoiceId: number) => { throw new Error('download failed'); }),
        getManualInvoices: async (_companyId: number) => ({ success: true, data: [{ id: 3, companyId: 5, invoiceNumber: 'INV-3', invoiceDate: '2025-12-01', amount: 123 }] })
      },
      classification: {
